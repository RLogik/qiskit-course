...
docker-itests:
    @- just _docker-build-and-interact "itests" "bot_itests"
...
api:
    ...
    @just build-misc
    @# first kill any dangling processes:
    @just kill-api-process
    @# start a new session
    @uvicorn src.api:app --port {{PORT}} &
    @# open in browser
    @{{PYTHON}} -m webbrowser '{{URL_API}}'
